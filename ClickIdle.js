...
var id = "ASCII?";
var name = "Farrer Smile";
var description = "baaaaaaaaaaa";
...
var init = () => {
    ...
    ///////////////////
    //// Story chapters
    function coreStory(num) {
        if (num == 0) {
            return "How how this game not where ffffff \n|\\|"
        }
    }

    chapter1 = theory.createStoryChapter(0, "My First Chapter", coreStory(0), () => true);
    chapter2 = theory.createStoryChapter(1, "My Second Chapter", "This is line 1 again,\nand this is line 2... again.\n\nNice again.", () => c2.level > 0);

    updateAvailability();

    var getHomePopup = ui.createPopup({
        title: "Home",
        content: ui.createStackLayout({
            children: [
                ui.createFrame({
                    heightRequest: 309,
                    cornerRadius: 0,
                    content: ui.createLabel({text: "You Have" + currency2.value + " Smiles.",
                        padding: Thickness(12, 2, 12, 2),
                        fontSize: 19
                    })
                }),
                ui.createLabel({
                    text: "Smiles this game for know!?",
                    horizontalTextAlignment: TextAlignment.CENTER,
                    fontAttributes: FontAttributes.BOLD,
                    fontSize: 18,
                    padding: Thickness(0, 18, 0, 18),
                }),
                ui.createButton({text: "Reset", onClicked: () => currency2.value = BigNumber.from(0)}),
                ui.createButton({text: "Close", onClicked: () => getHomePopup.hide()})
            ]
        })
    });

    var getThinkPopup = ui.createPopup({
        title: "😉",
        content: ui.createStackLayout({
            children: [
                ui.createLabel({
                ...
                ui.createButton({text: "Close", onClicked: () => getThinkPopup.hide()})
            ]
        })
    });

    var getVariablePopup = ui.createPopup({
        title: "Variable",
        content: ui.createStackLayout({
            children: [
                ui.createFrame({
                    heightRequest: 309,
                    cornerRadius: 0,
                    content: ui.createLabel({text: "you have 3 functions, core, piSymbol, coreStory",
                        padding: Thickness(12, 2, 12, 2),
                        fontSize: 14,
                    })
                }),
